...
py::enum_<BOOST_PP_CAT(T_QUALNAME(T_NAMETUPLE(ENUM_DEF)), _enum)>(m, BOOST_PP_STRINGIZE(BOOST_PP_CAT(T_NAME(T_NAMETUPLE(ENUM_DEF)), _enum)))\
BOOST_PP_SEQ_FOR_EACH(E_PY_VALUE, BOOST_PP_CAT(T_QUALNAME(T_NAMETUPLE(ENUM_DEF)), _enum), T_DIRECT_MEMBERS(ENUM_DEF));\
...
.def(py::init([](const BOOST_PP_CAT(T_QUALNAME(T_NAMETUPLE(ENUM_DEF)), _enum)& other){\
...
.def_property_readonly_static("enum", [](py::object /* self */){\
    return py::type::of<BOOST_PP_CAT(T_QUALNAME(T_NAMETUPLE(ENUM_DEF)), _enum)>();\
})
